...
function scoreVoiceFor(langBCP47, voice) {
  ...
  if (base.startsWith("ko")) {
    ...
    if (name.includes("yuna") || name.includes("yuri") || name.includes("nara"))
      score += 3;
...
export default function ResultPanel() {
  ...
  const updateBounds = useCallback(async () => {
    ...
    if (containerElRef.current) {
      ...
      moveablePanelRef.current?.setBounds({
        ...
        bottom:
          scrollTop +
          (1 + displaySettingRef.current.floatingData.height) *
            window.innerHeight -
          64,
      });
    }
  ...
  useEffect(() => {
    ...
    getOrSetDefaultSettings(
      ["languageSetting", "DefaultTranslator"],
      DEFAULT_SETTINGS,
    ).then(async (result) => {
    ...
    channel.on("command", (detail) => {
      switch (detail.command) {
        case "fix_result_frame":
          getOrSetDefaultSettings("fixSetting", DEFAULT_SETTINGS).then(
            (result) => {
              setPanelFix(!result.fixSetting);
  ...
  const onDisplayStatusChange = useCallback((panelEl) => {
    ...
    moveablePanelRef.current
      .on("dragStart", ({ set, stop, inputEvent }) => {
        if (inputEvent) {
          const path =
            inputEvent.path ||
            (inputEvent.composedPath && inputEvent.composedPath());
    ...
    moveablePanelRef.current
      ...
      .on("resizeEnd", ({ translate, width, height, inputEvent, target }) => {
        ...
        if (inputEvent) {
          if (displaySettingRef.current.type === "floating") {
            displaySettingRef.current.floatingData.width =
              width / window.innerWidth;
            displaySettingRef.current.floatingData.height =
  ...
  useEffect(() => {
    if (displaySettingRef.current.type === "floating") {
      ...
      if (!userMovedRef.current && moveablePanelRef.current) {
        const width =
          displaySettingRef.current.floatingData.width * window.innerWidth;
        const height =
          displaySettingRef.current.floatingData.height * window.innerHeight;
        let base = null;
        if (
          contentRef.current.position &&
          Array.isArray(contentRef.current.position)
        ) {
          base = [
            contentRef.current.position[0],
            contentRef.current.position[1],
          ];
  ...
  async function showPanel() {
    ...
    if (displaySettingRef.current.type === "floating") {
      ...
      if (contentRef.current.position) {
        ...
        position = [
          (1 - displaySettingRef.current.floatingData.width) *
            window.innerWidth -
            (hasScrollbar() ? scrollbarWidth : 0),
          0,
        ];
  ...
  function showFloatingPanel() {
    ...
    if (
      ...
      if (actualHeight !== headH && panelHeight > actualHeight)
  ...
  function showFixedPanel() {
    ...
    if (displaySettingRef.current.fixedData.position === "right")
      offsetLeft =
        window.innerWidth - width - (hasScrollbar() ? scrollbarWidth : 0);
    getOrSetDefaultSettings("LayoutSettings", DEFAULT_SETTINGS).then(
      async (result) => {
  ...
  function getDisplaySetting() {
    return new Promise((resolve) => {
      getOrSetDefaultSettings("DisplaySetting", DEFAULT_SETTINGS).then(
        (result) => {
    ...
    <Fragment>
      ...
      {highlight.show && (
        <Highlight
          style={{
            width:
              displaySettingRef.current.fixedData.width * window.innerWidth,
...
const PanelBgDark = "#202124";
const HeadIconBgDark = "#292a2d";
const HeadIconFillDark = "#9aa0a6";
...
const Panel = styled.div`
  ...
  z-index: ${MaxZIndex};
  border-radius: ${(props) =>
    props.displayType === "floating" ? PanelBorderRadius : 0};
  overflow: visible;
  box-shadow: 0px 8px 12px 5px rgba(0, 0, 0, 0.25);
  background: ${(props) =>
    props.theme?.isDark ? PanelBgDark : "rgba(235, 235, 235, 1)"};
  ...
  font-size: 16px;
  font-weight: normal;
  color: ${(props) => (props.theme?.isDark ? "#e8eaed" : "black")};
  line-height: 1;
...
const HeadIcon = styled.div`
  ...
  background-color: ${(props) =>
    props.theme?.isDark ? HeadIconBgDark : "rgba(255, 255, 255, 0.8)"};
  border-radius: 15px;

  svg {
    fill: ${(props) => (props.theme?.isDark ? HeadIconFillDark : "#8e8e93")};
    width: 16px;
    height: 16px;
    display: block;
