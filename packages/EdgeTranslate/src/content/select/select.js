...
const isOwnIframe = (() => {
  try {
    return window.frameElement && window.frameElement.id === "edge-translate-button";
  } catch (e) {
    return false;
  }
})();
 ...
 */
let translationButtonContainer;
if (!isOwnIframe) {
...
function renderButton() {
  ...
  Object.assign(
    translationButtonContainer.contentDocument?.body.style || {},
    CleanStyle,
  );
...
if (!isOwnIframe) {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    ButtonPositionSetting =
      changes.LayoutSettings.newValue.SelectTranslatePosition;
  });
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    async function selectTranslate(event, isDoubleClick = false) {
      ...
      getOrSetDefaultSettings("OtherSettings", DEFAULT_SETTINGS).then(
        (result) => {
...
function shouldTranslate() {
  ...
  if (BROWSER_ENV === "firefox")
    ...
    return (
      ...
      !(
        ...
        window.translateResult.originalText === selectionText
      )
    );
  ...
  return (
    selectionText.length > 0 &&
    (filterNode(selectionObject.anchorNode) ||
      filterNode(selectionObject.focusNode)) &&
    // Do not re-translate translated text.
    !(
      window.isDisplayingResult &&
      window.translateResult.originalText === selectionText
...
function isInBlacklist() {
  return getOrSetDefaultSettings("blacklist", DEFAULT_SETTINGS).then(
    (result) => {
...
function cancelPageTranslate() {
  ...
  if (frame !== null && frame !== undefined) {
    let cancelButton = frame.contentDocument.getElementById(
