# EdgeTranslate333 (v3.4.0)

本專案已依照 Manifest V3 全面重構，符合現行瀏覽器政策與建置流程，重點在提升穩定性與可維護性。

- 主題模式支援（自動 / 淺色 / 深色，預設跟隨系統）
- 支援 Google TTS 發音音訊直接播放
- 改進的 UI/UX 設計
---
...
- TTS / 主題偏好重設：可在設定頁一鍵清除舊環境殘留的語音角色與主題覆寫設定。

### 已知問題

- `Chrome` 資源耗盡與瀏覽器崩潰問題目前仍未徹底解決。
- 在長時間使用、動態網站、整頁翻譯、或反覆觸發側邊翻譯面板的情境下，仍可能出現記憶體暴增、頁面卡死、面板不出現、或整個 Chrome 無回應。
- `v3.4.0` 沒有宣稱已修復這個根因；這一版主要先回退近期造成功能回歸的 runtime 優化，保留基線功能，後續仍需要進一步做 profiling 與 leak/root-cause 追查。
